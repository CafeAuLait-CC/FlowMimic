...
def _condition_error(
    ...
    return {
        ...
        "e2d_slack": float(np.mean(slack_err)) if slack_err else float(np.mean(strict_err)),
        "e2d_strict_median": float(np.median(strict_err)),
        "e2d_strict_p90": float(np.percentile(strict_err, 90)),
        "e2d_slack_median": float(np.median(slack_err)) if slack_err else float(np.median(strict_err)),
        "e2d_slack_p90": float(np.percentile(slack_err, 90)) if slack_err else float(np.percentile(strict_err, 90)),
...
def _collect_features(motion):
    return motion.mean(dim=1).cpu().numpy()
...
def _load_cond_batch(
    ...
    return k2d_batch, vis_batch, tau_batch, mask_batch, k2d_list, vis_list
...
def _generate_batch(
    ...
    with torch.inference_mode():
        ...
        if compute_feat:
            feats = _collect_features(z_hat)
    if isinstance(device, torch.device) and device.type == "cuda":
        torch.cuda.synchronize()
    flow_time = time.perf_counter() - t_start
    x_hat_np = z_hat.cpu().numpy()
    cont_end = LAYOUT_SLICES["feet_contact"][0]
    x_hat_denorm = x_hat_np.copy()
    x_hat_denorm[..., :cont_end] = x_hat_denorm[..., :cont_end] * std + mean
    contact_logits = x_hat_np[..., LAYOUT_SLICES["feet_contact"][0] : LAYOUT_SLICES["feet_contact"][1]]
    joints = ik263_to_smpl22(x_hat_denorm)
    full_time = time.perf_counter() - t_start
    return x_hat_np, joints, contact_logits, feats, flow_time, full_time
...
def evaluate_dataset(
    ...
    for batch in tqdm(dataloader, desc=f"{name} eval", leave=False):
        ...
        if compute_dist:
            ...
            feats_ref.append(_collect_features(motion))

        seen += batch_size

    summary = {}
    if results:
        keys = [k for k in results[0].keys() if k not in ("dataset", "steps")]
        for k in keys:
