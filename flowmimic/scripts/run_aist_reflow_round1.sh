...
RUN_NAME="${RUN_NAME:-vqflow_aist_zq16_reflow1_strict_260820}"
WANDB_ID="${WANDB_ID:-rflow1a1}"
...
MAX_UPDATES="${MAX_UPDATES:-17300}"
WANDB_MODE="${WANDB_MODE:-online}"
WANDB_RESUME="${WANDB_RESUME:-allow}"

SOURCE_CKPT="${SOURCE_CKPT:-checkpoints/flow/vqflow_aist_zq16_unified_sparse_cfg5_260728/flow_round0_update68220.pt}"
...
for required_path in "$SOURCE_CKPT" "$VAE_CKPT" "$LATENT_STATS" "$EVAL_MANIFEST"; do
  if [[ ! -s "$required_path" ]]; then
    echo "Required reflow input does not exist or is empty: $required_path" >&2
    exit 1
  fi
done
...
if [[ -s "$LAST_GOOD" ]]; then
  ...
  START_ARGS=(--init-from "$SOURCE_CKPT" --init-use-ema)
fi
...
env \
  ...
  --wandb-tags aist,vqvae,zq16,reflow1,strict,round0-ema-teacher,velocity-only,true-null,cfg-drop5,sparse-patterns,joint-drop5,no-frame-mask,no-flow-regularization,ema099,test-boundary-gap-k7,guidance2p5
