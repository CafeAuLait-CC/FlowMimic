...
RUN_NAME="${RUN_NAME:-vqflow_aist_zq16_reflow1_cfgbranch_w010_260821}"
WANDB_ID="${WANDB_ID:-r1cfgbp1}"
CURRICULUM="${CURRICULUM:-reflow_round1_cfg_branch_preserve}"
BRANCH_TAG="${BRANCH_TAG:-w010}"
WANDB_TAGS="${WANDB_TAGS:-aist,vqvae,zq16,reflow1,cfg-aware,guidance2p5,branch-preservation,${BRANCH_TAG},local-teacher-velocity,round0-ema-teacher,cfg-aware-ema-resume,velocity-only,true-null,sparse-patterns,joint-drop5,no-frame-mask,no-flow-regularization,ema099,test-boundary-gap-k7}"
GPU_IDS="${CUDA_VISIBLE_DEVICES:-0,1}"
NPROC_PER_NODE="${NPROC_PER_NODE:-2}"
MASTER_PORT="${MASTER_PORT:-29822}"
...
MAX_UPDATES="${MAX_UPDATES:-12110}"
WANDB_MODE="${WANDB_MODE:-online}"
WANDB_RESUME="${WANDB_RESUME:-allow}"

SOURCE_CKPT="${SOURCE_CKPT:-checkpoints/flow/vqflow_aist_zq16_reflow1_cfgaware_260821/flow_round1_last_good.pt}"
...
for required_path in "$SOURCE_CKPT" "$TEACHER_CKPT" "$VAE_CKPT" "$LATENT_STATS" "$EVAL_MANIFEST"; do
  if [[ ! -s "$required_path" ]]; then
    echo "Required CFG branch-preservation input does not exist or is empty: $required_path" >&2
    exit 1
  fi
done

if [[ ! -s "$LAST_GOOD" ]]; then
  cp "$SOURCE_CKPT" "$LAST_GOOD"
fi
