...
case "$MODE" in
  sparse)
    DEFAULT_RUN_NAME="vqflow_aist_zq16_sparse_pattern_phase1_260720"
    DEFAULT_WANDB_ID="sp1mix20"
    CURRICULUM="sparse_pattern_phase1"
    ;;
  control)
    DEFAULT_RUN_NAME="vqflow_aist_zq16_sparse_pattern_control_260720"
    DEFAULT_WANDB_ID="sp1ctl20"
    CURRICULUM="sparse_pattern_phase1_control"
    ;;
  *)
    echo "MODE must be sparse or control, got: $MODE" >&2
    exit 2
    ;;
esac
...
SOURCE_CHECKPOINT="${SOURCE_CHECKPOINT:-checkpoints/flow/vqflow_aist_zq16_unified_round0_260718/flow_round0_update51900.pt}"
EVAL_MANIFEST="${EVAL_MANIFEST:-training_logs/phase0_sparse_condition_zero_shot_20260720/manifests/boundary_gap_k14.json}"
...
env \
  ...
  --wandb-group VQFlow-Sparse-Pattern-Phase1 \
  --wandb-name "$RUN_NAME" \
  --wandb-id "$WANDB_ID" \
  --wandb-resume allow \
  --wandb-mode "$WANDB_MODE" \
  --wandb-tags "aist,vqvae,zq16,phase1,${MODE},pattern-curriculum,shared-camera,joint-drop5,no-cfg,no-frame-mask,no-flow-smooth,ema099,boundary-gap-eval,k14,eval470x3,steps50"
