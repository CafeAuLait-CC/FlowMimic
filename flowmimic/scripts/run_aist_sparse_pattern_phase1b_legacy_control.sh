...
RUN_NAME="${RUN_NAME:-vqflow_aist_zq16_sparse_pattern_phase1b_legacy_control_rngmatched_260721}"
WANDB_ID="${WANDB_ID:-sp1blc21m}"
...
printf '[%s] GPU=%s curriculum=%s max_updates=%s resume=%s reset_optimizer=%s\n' \
  "$(date -Is)" "$GPU_ID" "sparse_pattern_phase1b_legacy_control" \
...
env \
  ...
  "${MAX_UPDATE_ARGS[@]}" \
  "${RESET_OPT_ARGS[@]}" \
  --curriculum sparse_pattern_phase1b_legacy_control \
  --batch-size 640 \
  --datasets AIST \
  --seq-len 196 \
  ...
  --wandb-tags "aist,vqvae,zq16,phase1b,legacy-control,rng-matched,no-relative-time,boundary-k7-quota,shared-camera,joint-drop5,no-cfg,no-frame-mask,no-flow-smooth,ema099,val-boundary-gap-eval,k7,eval3,steps50"
