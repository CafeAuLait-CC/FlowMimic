...
RUN_NAME="${RUN_NAME:-vqflow_aist_zq16_sparse_pattern_phase1c_slot_adapter_260722}"
WANDB_ID="${WANDB_ID:-sp1csa22}"
...
printf '[%s] GPU=%s curriculum=%s max_updates=%s resume=%s reset_optimizer=%s\n' \
  "$(date -Is)" "$GPU_ID" "sparse_pattern_phase1c_latent_slot_adapter" \
...
env \
  ...
  "${MAX_UPDATE_ARGS[@]}" \
  "${RESET_OPT_ARGS[@]}" \
  --curriculum sparse_pattern_phase1c_latent_slot_adapter \
  --batch-size 640 \
  --datasets AIST \
  --seq-len 196 \
  ...
  --wandb-group VQFlow-Sparse-Pattern-Phase1C \
  --wandb-name "$RUN_NAME" \
  --wandb-id "$WANDB_ID" \
  --wandb-resume allow \
  --wandb-mode "$WANDB_MODE" \
  --wandb-tags "aist,vqvae,zq16,phase1c,latent-slot-adapter,vq-query-init,adapter-lr10x,no-relative-time,boundary-k7-quota,shared-camera,joint-drop5,no-cfg,no-frame-mask,no-flow-smooth,ema099,val-boundary-gap-eval,k7,eval3,steps50"
