...
RUN_NAME="${RUN_NAME:-vqflow_aist_zq16_unified_sparse_cfg5_260728}"
WANDB_ID="${WANDB_ID:-ufcfg528}"
...
env \
  ...
  --wandb-group VQFlow-Unified-Sparse-CFG \
  --wandb-name "$RUN_NAME" \
  --wandb-id "$WANDB_ID" \
  --wandb-resume "$WANDB_RESUME" \
  --wandb-mode "$WANDB_MODE" \
  --wandb-tags aist,vqvae,zq16,unified,sparse-patterns,true-null,cfg-drop5,cfg-ramp5,shared-camera,joint-drop5,no-frame-mask,no-flow-smooth,ema099,test-boundary-gap-k7,guidance2p5,eval470x3,steps50
