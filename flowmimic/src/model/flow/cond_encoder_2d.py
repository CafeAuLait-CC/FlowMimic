...
def normalize_keypoints(k2d, vis_mask=None, pelvis_idx=8, eps=1e-6):
    # k2d: [B, T_c, J, 2]
    if vis_mask is None:
    ...
    center = torch.where(pelvis_vis > 0.5, pelvis, (k * vis_mask.unsqueeze(-1)).sum(dim=2, keepdim=True))
...
class CondEncoder2D(nn.Module):
    ...
    def forward(self, k2d, tau_cond, vis_mask=None, mask_cond=None):
        # k2d: [B, T_c, J, 2], tau_cond: [B, T_c] or [T_c]
        k2d_norm, vis_mask = normalize_keypoints(k2d, vis_mask=vis_mask)
