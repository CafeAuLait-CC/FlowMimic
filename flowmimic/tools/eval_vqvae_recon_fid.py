...
from flowmimic.src.metrics.reconstruction_metrics import physical_reconstruction_metrics
...
def main() -> None:
    ...
    metrics.update(
        physical_reconstruction_metrics(
    ...
    metrics.update(
        {
            ...
            "aggregate_code_utilization": float(
                aggregate_codes_used / max(vae.quantizer.num_codes, 1)
            ),
        }
    )
