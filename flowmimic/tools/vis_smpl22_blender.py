...
def make_material(name: str, rgba):
    ...
    if len(node_principled.inputs) > 0:
...
def build_rig_objects(kps0, chains, col, sphere_mat, bone_mat):
    ...
    J = len(kps0)
    spheres = [
        create_sphere(f"kp_{i:03d}", kps0[i], SPHERE_RADIUS, col, sphere_mat)
        for i in range(J)
    ]
    ...
    for chain in chains:
        for a, b in zip(chain[:-1], chain[1:]):
            if 0 <= a < J and 0 <= b < J:
                obj = create_cylinder(
                    f"bone_{a}_{b}", kps0[a], kps0[b], BONE_RADIUS, col, bone_mat
                )
                if obj is not None:
                    bones.append(obj)
...
sph_b, bones_b, pairs_b = build_rig_objects(
    kps0_before, CHAINS, col_b, mat_b_sphere, mat_b_bone
)
sph_a, bones_a, pairs_a = build_rig_objects(
    kps0_after, CHAINS, col_a, mat_a_sphere, mat_a_bone
)
...
print(
    f"Done. Two motions animated: T={T}, J={J}. Collections: {COL_BEFORE}, {COL_AFTER}"
)
