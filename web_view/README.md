...
- After an AIST++ test/validation motion is generated at length 196, use **Export comparison .blend** in the 3D Motion header. The modal randomly selects a camera-matched description, supports rerolling or editing it, and accepts three distinct clip-local StickMotion sketch frames with source-frame offsets.
...
- When the job completes, the page adds MLD and StickMotion motion viewers plus text, sketch, and locus conditions, and enables the `.blend` download.
