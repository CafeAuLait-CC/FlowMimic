...
- Comparison creation runs MLD, StickMotion, and Blender as a persisted background job under `output/flow/comparison_jobs/`. It reuses the exact FlowMimic motion already displayed in the web view and builds the `.blend` with the currently selected skeleton or rigged-model visualization. StickMotion uses the retrained no-locus checkpoint `runs/stickmotion/human_ml3d/aist_remodiffuse_no_locus_260730/epoch=555-step=24132.ckpt`; its ground-truth trajectory branch is disabled. MLD and StickMotion viewer results remain visible when FlowMimic regenerates the same sample path and clip start; the old `.blend` download is hidden because that scene contains the previous FlowMimic motion.
